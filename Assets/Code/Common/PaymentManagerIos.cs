...
public class PaymentManagerIos {
	...
	public static void init() {
			...
			if(!IsInitialized) {
				...
				//Billing tab.
				IOSInAppPurchaseManager.Instance.AddProductId(INFINITE);
	...
	{
		IOSNativePopUpManager.showMessage("Purchasing", "Wait a few seconds...");
	...
	private static void UnlockProducts(string productIdentifier) {
		...
		case INFINITE:
			...
			GameCenterManager.SubmitAchievement(100f, Achievements.NOPUBLI);
			Analytics.Transaction(productIdentifier, (decimal)IOSInAppPurchaseManager.Instance.GetProductById(PaymentManagerIos.INFINITE).Price, IOSInAppPurchaseManager.Instance.GetProductById(PaymentManagerIos.INFINITE).CurrencyCode, null, null);
			break;
